...
export default function AnalyzePage() {
  ...
  const detectAlgorithm = (code) => {
    ...
    if (codeLower.includes('sort') || codeLower.includes('bubble') || codeLower.includes('quick') || codeLower.includes('merge') || codeLower.includes('heap')) {
      return 'sorting';
    } else if (codeLower.includes('search') || codeLower.includes('binary') || codeLower.includes('linear') || codeLower.includes('find')) {
      return 'searching';
    } else if (codeLower.includes('graph') || codeLower.includes('bfs') || codeLower.includes('dfs') || codeLower.includes('traversal')) {
      return 'graph_traversal';
    } else if (codeLower.includes('tree') || codeLower.includes('inorder') || codeLower.includes('preorder') || codeLower.includes('postorder')) {
      return 'tree_traversal';
    } else if (codeLower.includes('dp') || codeLower.includes('dynamic') || codeLower.includes('memoization')) {
      return 'dynamic_programming';
    } else if (codeLower.includes('palindrome') || codeLower.includes('stack') || codeLower.includes('queue')) {
  ...
  const handleSelectExample = (exampleCode, exampleLanguage) => {
    ...
    setSnackbar({ open: true, message: `${exampleLanguage.charAt(0).toUpperCase() + exampleLanguage.slice(1)} example loaded! Click Analyze to visualize.`, severity: 'success' });
  };

  const handleAnalyze = async () => {
    if (!code.trim()) {
      setSnackbar({ open: true, message: 'Please enter some code to analyze.', severity: 'warning' });
      return;
    }
    ...
    try {
      ...
      if (!res.data.success && res.data.error && res.data.error.includes('Input required')) {
        if (res.data.ast_analysis?.input_schema) {
          setInputSchema(res.data.ast_analysis.input_schema);
          setError('Input required for this code. Please provide the required input below.');
          setSnackbar({ open: true, message: 'Input required! Please fill in the input fields below.', severity: 'info' });
          return;
        }
      }

      // If we have input schema but no input values, show the input form
      if (res.data?.ast_analysis?.input_required && res.data?.ast_analysis?.input_schema && inputArr.length === 0) {
        setInputSchema(res.data.ast_analysis.input_schema);
        setError('Input required for this code. Please provide the required input below.');
        setSnackbar({ open: true, message: 'Input required! Please fill in the input fields below.', severity: 'info' });
        return;
      ...
      if (res.data.success) {
        setResult(res.data);
        setSnackbar({ open: true, message: 'Analysis complete! Explore the results below.', severity: 'success' });
      } else {
        // Handle different types of errors
        if (res.data.error && res.data.error.includes('Input required')) {
          if (res.data.ast_analysis?.input_schema) {
            setInputSchema(res.data.ast_analysis.input_schema);
            setError('Input required for this code. Please provide the required input below.');
            setSnackbar({ open: true, message: 'Input required! Please fill in the input fields below.', severity: 'info' });
          } else {
            setError('Input required but no schema provided. Please check your code.');
            setSnackbar({ open: true, message: 'Input detection failed. Please check your code.', severity: 'error' });
          }
        } else {
          setError(res.data.error || 'Analysis failed');
          setSnackbar({ open: true, message: 'Analysis failed. Please check your code and try again.', severity: 'error' });
        }
      }
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    setSnackbar({ open: true, message: 'Analysis exported successfully!', severity: 'success' });
  };
  ...
  const handleLoadExample = () => {
    ...
    if (example) {
      ...
      setSnackbar({ open: true, message: `${language.charAt(0).toUpperCase() + language.slice(1)} example loaded! Click Analyze to visualize.`, severity: 'success' });
    } else {
      setSnackbar({ open: true, message: `No example found for ${language}.`, severity: 'warning' });
    }
  };
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Box sx={{ display: 'flex', gap: 2, flexWrap: 'wrap', mb: 3 }}>
        <Button startIcon={<PlayArrow />} variant="outlined">Quick Run</Button>
        <Button startIcon={<Share />} variant="outlined">Share</Button>
        <Button startIcon={<Download />} variant="outlined">Download</Button>
        <Button onClick={handleSelectExample} variant="outlined">Select Example</Button>
        ...
        <Grid item xs={12} lg={4}>
          <Paper elevation={3} sx={{ p: 3, height: 'fit-content' }}>
            ...
            <Box display="flex" alignItems="center" gap={1} mb={2}>
              ...
              <Typography variant="body2">Memory usage is analyzed below.</Typography>
            </Box>
                  ...
                  <CardContent>
                    ...
                    <Box sx={{ mb: 2 }}>
                      <Typography variant="body2" color="textSecondary">
                        Time Complexity: {result.complexity_analysis?.time || 'N/A'}
                      </Typography>
                      <Typography variant="body2" color="textSecondary">
                        Space Complexity: {result.complexity_analysis?.space || 'N/A'}
                      </Typography>
                    </Box>
          ...
          {inputSchema && Array.isArray(inputSchema) && inputSchema.length > 0 ? (
            <Box>
              ...
              {inputSchema.map((field, idx) => (
                <TextField
                  key={field.name}
                  label={field.name + (field.type !== 'unknown' ? ` (${field.type})` : '')}
                  value={inputValues[field.name] || ''}
                  onChange={e => setInputValues(v => ({ ...v, [field.name]: e.target.value }))}
                  fullWidth
                  placeholder={field.example || (field.type === 'array' ? '[1,2,3]' : field.type === 'number' ? '42' : 'Enter value')}
                  helperText={field.type === 'array' ? 'Enter as JSON array, e.g., [1,2,3]' : 
                             field.type === 'number' ? 'Enter a number' : 
                             field.type === 'string' ? 'Enter text' : 'Enter value'}
                  sx={{ mb: 2 }}
                  variant="outlined"
                />
              ))}
              <Box display="flex" gap={2}>
                ...
                >
                  {loading ? <CircularProgress size={20} /> : 'Analyze with Input'}
                </Button>
              </Box>
            </Box>
          ) : (
            <Box>
              ...
              <Typography variant="body2" color="#666">
                The analyzer will automatically detect if input is needed based on your code.
              </Typography>
            </Box>
          )}
        </Paper>
      ...
      {result && !loading && !error && (
        <Paper sx={{ p: 3, mt: 3, backgroundColor: '#fff' }}>
          ...
          {activeTab === 1 && (
            <Box>
              <Typography variant="h6" gutterBottom>AST Analysis</Typography>
          ...
          {activeTab === 4 && (
            <Box>
              <Typography variant="h6" gutterBottom>Data Structures</Typography>
              <DataStructureVisualizer dataStructures={result.execution_trace?.[0]?.data_structures || {}} />
            </Box>
          )}
      ...
      <Dialog open={showOnboarding} onClose={() => setShowOnboarding(false)} maxWidth="md" fullWidth>
        ...
        <DialogContent dividers>
          ...
          <Box component="ol" sx={{ pl: 2 }}>
            ...
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Select your programming language</strong> (Python, Java, C++, etc.)
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Click "Analyze Code"</strong> to start the visualization
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>If input is required</strong>, the system will automatically detect and prompt you
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Explore the results</strong> through multiple tabs: step-by-step execution, AST, complexity, and more
            </Typography>
          </Box>
          ...
          <Box component="ul" sx={{ pl: 2 }}>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Step-by-step execution</strong> with variable tracking and data structure visualization
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Algorithm detection</strong> and specialized visualizations for sorting, searching, graphs, etc.
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
              <strong>Complexity analysis</strong> with time and space complexity estimates
            </Typography>
            <Typography component="li" variant="body1" sx={{ mb: 1 }}>
      ...
      <Dialog open={showShareDialog} onClose={() => setShowShareDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Typography gutterBottom>
            Share your code analysis with others. The link will include your code, input, and results.
          </Typography>
        </DialogContent>
        <DialogActions>
